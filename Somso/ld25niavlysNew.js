...
function Villain(x, y, player)
{
	...
	this.die = function()
	{
		...
		if(this.playerControlled)
		{
			log("You disappeared in the dark depths of… something.");
			running = false;
...
function init()
{
	...
	$(document).ready(function()
	{
		addEventListener("mousemove", function(e)
		{
			...
			/*if(oscillator)
			{
				var aaa = Math.sqrt((mouseX-V.x)^2+(mouseY-V.y)^2)*100;
				//$("#log").html(aaa);
				oscillator.frequency.value = (V.vx+V.vy)*4000;
			}*/
		}
		);
	});
...
function checkCompleted()
{
	...
	if(completed)
	{
		...
		//log("Completed : "+nbEprev"+"+nbE+"="+nbE+nbEprev);
...
function drawTimeout()
{
	...
	grdTimeout.addColorStop(0, "green");
	grdTimeout.addColorStop(0.33, "yellow");
	grdTimeout.addColorStop(0.66, "orange");
	grdTimeout.addColorStop(1, "red");
